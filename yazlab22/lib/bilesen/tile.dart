...
class _TileState extends State<Tile> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Consumer<Controller>(builder: (_, notifier, __) {
      ...
      } else if (widget.index == notifier.clueIndex) {  // Check if this tile is the clue tile
        text = notifier.answer[notifier.clueIndex];  // Display the clue letter
        fontColor = Colors.white;  // Make clue letter distinct, e.g., blue color
        _backgroundColor = Color.fromARGB(255, 203, 68, 221);  // Lighter background for the clue
      }

    // Burada Tile'in index'i kontrol edilir ve açık olarak işaretlenmişse harf gösterilir
    if (notifier.revealedIndexes.containsKey(widget.index)) {
      text = notifier.correctWord[widget.index % notifier.letterCount]; // Doğru harfi alır
      fontColor = Colors.white;  // İpucu harfin rengini mavi yapar
      _backgroundColor = Color.fromARGB(255, 234, 92, 139);  // Arka planı açık mavi yapar
    }
